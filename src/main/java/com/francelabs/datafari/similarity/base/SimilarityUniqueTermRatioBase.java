 ...
 * @lucene.experimental
 */
public abstract class SimilarityUniqueTermRatioBase extends Similarity {
  /**
   ...
   * implicit.)
   */
  public SimilarityUniqueTermRatioBase() {
  }
  ...
  private static final float[] RATIO_TABLE = new float[256];

  static {
    for (int i = 1; i < 256; i++) {
      RATIO_TABLE[i] = SmallFloat.byte315ToFloat((byte) i);
    }
  }
  ...
  public final long computeNorm(FieldInvertState state) {
    final int numTerms, numUniqueTerms;
    ...
    return SmallFloat.floatToByte315((float)numUniqueTerms / numTerms);
  }
   ...
   * {@link SimilarityUniqueTermCountBase#score(BasicStats, float, float)} and
   * {@link SimilarityUniqueTermCountBase#explain(BasicStats, int, Explanation, float)},
   * respectively.
   */
  final class BasicSimScorer extends SimScorer {
    ...
    public float score(int doc, float freq) throws IOException {
      ...
      return SimilarityUniqueTermRatioBase.this.score(stats, freq, getLengthValue(doc));
    }

    @Override
    public Explanation explain(int doc, Explanation freq) throws IOException {
      return SimilarityUniqueTermRatioBase.this.explain(stats, doc, freq, getLengthValue(doc));
    }
