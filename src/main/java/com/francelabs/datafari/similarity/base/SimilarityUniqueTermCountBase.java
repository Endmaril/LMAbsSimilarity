 ...
 */
package com.francelabs.datafari.similarity.base;
 ...
 * @lucene.experimental
 */
public abstract class SimilarityUniqueTermCountBase extends Similarity {
  /**
   ...
   * implicit.)
   */
  public SimilarityUniqueTermCountBase() {
  }
   ...
   * @param uniqueTermCount The number of unique terms.
   * @return the score.
   */
  protected abstract float score(BasicStats stats, float freq, float uniqueTermCount);

  /**
   * Subclasses should implement this method to explain the score. {@code expl}
   * already contains the score, the name of the class and the doc id, as well as
   * the term frequency and its explanation; subclasses can add additional clauses
   * to explain details of their scoring formulae.
   * <p>
   ...
   * {@link #score(BasicStats, float, float)} method) and the explanation for the
   * term frequency. Subclasses content with this format may add additional
   * details in {@link #explain(List, BasicStats, int, float, float)}.
   * 
   * @param stats  the corpus level statistics.
   * @param doc    the document id.
  ...
  protected Explanation explain(BasicStats stats, int doc, Explanation freq, float docLen) {
    ...
    return Explanation.match(score(stats, freq.getValue(), docLen),
  ...
  public final SimScorer simScorer(SimWeight stats, LeafReaderContext context) throws IOException {
    ...
    if (stats instanceof MultiSimilarity.MultiStats) {
      ...
      for (int i = 0; i < subScorers.length; i++) {
        ...
        subScorers[i] = new BasicSimScorer(basicstats, indexCreatedVersionMajor,
  ...
  /**
   * Subclasses must override this method to return the name of the Similarity and
   * preferably the values of parameters (if any) as well.
   */
  @Override
  ...
  /** Computes docLength / uniqueTermCount (Similarity should be decreasing 
   * with respect to the norm so we compute here the inverse of the expected
   * measure so that the final measure can be the inverse of the norm and thus
   * will be decreasing with respect to the norm). */
  @Override
  public final long computeNorm(FieldInvertState state) {
    final int numUniqueTerms;
    numUniqueTerms = state.getUniqueTermCount();

    return SmallFloat.intToByte4(numUniqueTerms);
  }
   ...
   * {@link SimilarityUniqueTermRatioBase#score(BasicStats, float, float)} and
   * {@link SimilarityUniqueTermRatioBase#explain(BasicStats, int, Explanation, float)},
   * respectively.
   */
  final class BasicSimScorer extends SimScorer {
    ...
    public float score(int doc, float freq) throws IOException {
      ...
      return SimilarityUniqueTermCountBase.this.score(stats, freq, getLengthValue(doc));
    }

    @Override
    public Explanation explain(int doc, Explanation freq) throws IOException {
      return SimilarityUniqueTermCountBase.this.explain(stats, doc, freq, getLengthValue(doc));
    }
